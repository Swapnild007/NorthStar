...
else if(activeTool==="timeline"){h+='<div class="terminal-panel"><div class="panel-head"><span>EVENT TIMELINE</span><em>CHRONOLOGICAL</em></div><div class="timeline-feed">';d.evidence.forEach((x,i)=>{h+='<div><span>EVT-'+String(i+1).padStart(2,"0")+'</span><i></i><p>'+esc(x)+'</p></div>'});h+='</div></div>'}
else{h+='<div class="terminal-panel"><div class="panel-head"><span>FINDINGS & EVIDENCE ARTIFACT</span><em>REPORT</em></div><div class="finding-brief"><h2>'+esc(d.deliverable)+'</h2><p>'+esc(d.success)+'</p></div><textarea class="lab-note cyber-report" data-lab-note="'+l.id+'" placeholder="Document observations, reasoning, conclusion, alternative explanation, uncertainty and next action...">'+esc(note)+'</textarea><div class="report-footer"><span>LOCAL EVIDENCE STORE · NO SERVER UPLOAD</span><button class="cyber-submit" data-lab-complete>'+(st==="completed"?"✓ LAB COMPLETED":"SUBMIT INVESTIGATION")+'</button></div></div>'}
h+='</main></div><div class="cyber-bottom"><span>'+toolNames[activeTool].toUpperCase()+' · CONTROLLED CYBER RANGE</span><div>'+(st==="completed"?"ASSESSMENT COMPLETE":"EVIDENCE-DRIVEN LEARNING")+' <b>•</b> LOCAL ONLY</div></div></section>';return h}, ai:()=>`<section class="fade">
