...
function cyberToolState(id){const w=labWorkspace(id);return {...w,selectedEvidence:Number.isInteger(w.selectedEvidence)?w.selectedEvidence:0,query:String(w.query||"")};}
...
function crEvidence(l,w){const q=w.query.toLowerCase();const rows=l.evidence.map((x,i)=>({x,i})).filter(o=>!q||o.x.toLowerCase().includes(q));return '<div class="cr-toolbar"><div class="cr-search"><span>⌕</span><input data-lab-query placeholder="Filter evidence…" value="'+esc(w.query)+'"></div><span class="cr-count">'+rows.length+'/'+l.evidence.length+' EVENTS</span></div><div class="cr-table">'+rows.map(o=>'<button class="cr-row '+(o.i===w.selectedEvidence?"selected":"")+'" data-lab-select="'+o.i+'"><span>'+String(o.i+1).padStart(2,"0")+'</span><code>'+esc(o.x)+'</code><i>INSPECT</i></button>').join("")+'</div>';}
function crInspector(l,w){const x=l.evidence[w.selectedEvidence]||l.evidence[0]||"No evidence selected";return crPanel("EVIDENCE INSPECTOR","SELECTED EVENT",'<div class="cr-inspector"><div class="cr-signal">EVENT '+String(w.selectedEvidence+1).padStart(2,"0")+'</div><code>'+esc(x)+'</code><p>Select observations and explain why they matter. NorthStar does not pre-label the answer for you.</p><div class="cr-facts"><span><b>TYPE</b> SIMULATED</span><span><b>SOURCE</b> LAB DATASET</span><span><b>TRUST</b> CONTROLLED</span></div></div>');}
function crInstrument(l,w){
 const id=l.id;
 if(id==="packet-recon"||id==="foundation-tcp"||id==="foundation-ip") return '<div class="cr-instrument-grid">'+crPanel(id==="packet-recon"?"PACKET ANALYZER":"NETWORK OBSERVATORY","PCAP // SIMULATED",'<div class="packet-map"><div class="endpoint"><b>CLIENT</b><span>TRAINING HOST</span></div><div class="wire"><i></i><span>TRAFFIC STREAM</span></div><div class="endpoint"><b>DESTINATION</b><span>SIMULATED</span></div></div>'+crEvidence(l,w),"wide")+crInspector(l,w)+'</div>';
 if(id==="web-surface"||id==="auth-boundary"){const routes=id==="auth-boundary"?["POST /login · 200 · AUTH","GET /profile · 200 · AUTH","GET /admin · 403 · USER","GET /admin · 200 · PRIVILEGED"]:["GET /login · 200 · PUBLIC","GET /search?q=training · 200 · INPUT","GET /api/profile · 401 · AUTH","GET /admin · 403 · RESTRICTED","GET /health · 200 · PUBLIC"];return '<div class="cr-instrument-grid"><div>'+crPanel("WEB SURFACE MAP","ROUTES // SIMULATED",'<div class="route-list">'+routes.map((x,i)=>'<button class="route-item '+(i===w.selectedEvidence%routes.length?"selected":"")+'" data-lab-select="'+i+'"><span>'+String(i+1).padStart(2,"0")+'</span><code>'+esc(x)+'</code><b>›</b></button>').join("")+'</div>',"wide")+crPanel("REQUEST CONSOLE","READ ONLY",'<div class="request-console"><div><span>METHOD</span><b>GET</b><span>STATE</span><b>SIMULATED</b></div><code>'+esc(routes[w.selectedEvidence%routes.length])+'</code><p>Observe the trust boundary before making a vulnerability claim.</p></div>')+'</div><div>'+crPanel("TRUST BOUNDARY","ACCESS MODEL",'<div class="trust-flow"><span>ANONYMOUS</span><i>→</i><span>AUTHENTICATED</span><i>→</i><span>PRIVILEGED</span></div>')+crEvidence(l,w)+'</div></div>';}
 if(id==="detection-drill"||id==="log-triage"||id==="dns-hunt"||id==="linux-process"){const labels=id==="detection-drill"?["BASELINE","BASELINE","BASELINE","INVESTIGATE","INVESTIGATE"]:id==="dns-hunt"?["BASELINE","BASELINE","ANOMALOUS","ANOMALOUS","ANOMALOUS","CONTEXT"]:["IDENTITY","ENDPOINT","NETWORK","ENDPOINT","NETWORK"];return '<div class="cr-instrument-grid">'+crPanel("SOC EVENT STREAM","TELEMETRY // SIMULATED",'<div class="soc-stream">'+l.evidence.map((x,i)=>'<button class="soc-event '+(i===w.selectedEvidence?"selected":"")+'" data-lab-select="'+i+'"><span>0'+(i+1)+'</span><i class="sev '+(labels[i]==="INVESTIGATE"||labels[i]==="ANOMALOUS"?"hot":"ok")+'"></i><code>'+esc(x)+'</code><b>'+esc(labels[i]||"EVENT")+'</b></button>').join("")+'</div>',"wide")+crPanel("ANALYST CONTEXT","CORRELATION",'<div class="context-cards"><div><b>ENTITY</b><span>CONTROLLED HOST</span></div><div><b>IDENTITY</b><span>SIMULATED USER</span></div><div><b>STATE</b><span>INVESTIGATION</span></div></div><div class="analyst-question">Why does this event matter in sequence rather than by itself?</div>')+'</div>';}
 if(id==="incident-room"||id==="mystery-investigation")return '<div class="cr-instrument-grid">'+crPanel("INCIDENT TIMELINE","MULTI-SOURCE // SIMULATED",'<div class="incident-timeline">'+l.evidence.map((x,i)=>'<button class="incident-event '+(i===w.selectedEvidence?"selected":"")+'" data-lab-select="'+i+'"><span>EVT-'+String(i+1).padStart(2,"0")+'</span><i></i><code>'+esc(x)+'</code></button>').join("")+'</div>',"wide")+crPanel("ENTITY GRAPH","RELATIONSHIPS",'<div class="entity-graph"><div class="entity-node">USER<span>SIMULATED IDENTITY</span></div><div class="entity-line"></div><div class="entity-node">ENDPOINT<span>CONTROLLED ASSET</span></div><div class="entity-line"></div><div class="entity-node">RESOURCE<span>SIMULATED</span></div></div>')+'</div>';
 return '<div class="cr-instrument-grid">'+crPanel("LAB TELEMETRY","CONTROLLED DATASET",crEvidence(l,w),"wide")+crInspector(l,w)+'</div>';
}
const views={
 home:()=>{
...
function bind(){
 ...
 if(labComplete)labComplete.onclick=()=>{const l=labs[state.selectedLab],d=LAB_DETAILS[l.id],w=labWorkspace(l.id);const answered=d.checkpoints.filter((_,i)=>String(w.answers?.[i]||"").trim()).length;if(answered<d.checkpoints.length||String(w.note||"").trim().length<40){alert("Complete all investigation checkpoints and write an evidence artifact before completing this lab.");return;}state.labState[state.selectedLab]="completed";persist();render()};
