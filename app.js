...
function labTerminalOutput(l,w,cmd){
 const c=String(cmd||"").trim(),d=getLabDetails(l.id),e=d.evidence||[],x=c.toLowerCase();
 if(!c)return "";
 if(x==="help")return ["AVAILABLE COMMANDS","  help","  status","  evidence","  inspect <n>","  filter <term>","  timeline","  connections","  process-tree","  headers","  cert","  iam","  image","  mail","  prompt","  findings","  whoami","  clear","This console is simulated. Commands never execute on a real system."].join("\n");
 ...
 if(x==="connections")return e.filter(v=>/→|connection|HTTPS|TCP|TLS|DNS/i.test(v)).map((v,i)=>"CONN-"+String(i+1).padStart(2,"0")+"  "+v).join("\n")||"No connection telemetry in this lab.";
 if(x==="process-tree")return e.filter(v=>/process|powershell|chrome|winword|explorer|agent|executable/i.test(v)).map((v,i)=>"PROC-"+String(i+1).padStart(2,"0")+"  "+v).join("\n")||"No process telemetry in this lab.";
 if(x==="headers")return e.filter(v=>/HTTP|Cookie|Set-Cookie|header|GET|POST|403|401|302/i.test(v)).map((v,i)=>"HTTP-"+String(i+1).padStart(2,"0")+"  "+v).join("\n")||"No HTTP telemetry in this lab.";
 if(x==="cert")return e.filter(v=>/TLS|certificate|issuer|CN=|SAN|validation|ClientHello|ServerHello/i.test(v)).join("\n")||"No certificate telemetry in this lab.";
 if(x==="iam")return e.filter(v=>/role=|AssumeRole|iam:|user=|source=|identity/i.test(v)).join("\n")||"No IAM telemetry in this lab.";
 if(x==="image")return e.filter(v=>/image=|container=|digest=|base=|port|runtime|package|user=/i.test(v)).join("\n")||"No container metadata in this lab.";
 if(x==="mail")return e.filter(v=>/From:|Return-Path|SPF|DKIM|DMARC|URL|email|message/i.test(v)).join("\n")||"No mail telemetry in this lab.";
 if(x==="prompt")return e.filter(v=>/SYSTEM|USER|TOOL|MODEL|prompt|policy|untrusted/i.test(v)).join("\n")||"No AI prompt telemetry in this lab.";
 if(x==="findings")return (w.locker||[]).length?["EVIDENCE LOCKER","",...(w.locker.map(n=>String(n+1).padStart(2,"0")+"  "+(e[n]||"")))].join("\n"):"Evidence locker is empty.";
 if(x.startsWith("filter ")){const term=c.slice(7).trim().toLowerCase();return e.map((v,i)=>({v,i})).filter(o=>o.v.toLowerCase().includes(term)).map(o=>"["+String(o.i+1).padStart(2,"0")+"] "+o.v).join("\n")||"No matching evidence.";}
 if(x.startsWith("inspect ")){const n=Number(x.split(/\s+/)[1]);return Number.isInteger(n)&&n>=1&&n<=e.length?["EVENT "+String(n).padStart(2,"0"),e[n-1],"","Analyst note: observation only. Explain why it matters before forming a conclusion."].join("\n"):"Usage: inspect <evidence number>";}
 if(x==="clear")return "__CLEAR__";
 return "command not found: "+c+"\nType 'help'.";
}
function crTerminal(l,w){
 const history=Array.isArray(w.terminalHistory)?w.terminalHistory:[],lines=history.flatMap(v=>["> "+v.cmd,v.out==="__CLEAR__"?"":v.out]).filter(Boolean);
 const output=lines.length?lines.map(v=>"<div>"+esc(v).replace(/\n/g,"<br>")+"</div>").join(""):'<div class="terminal-dim">CyberRange shell ready. Type <b>help</b> to inspect the safe command set.</div>';
 return crPanel("SECURE TERMINAL","SIMULATED SHELL",'<div class="cr-terminal"><div class="cr-terminal-bar"><span><i></i><i></i><i></i></span><b>northstar@cyberrange:~</b><em>NO EXECUTION</em></div><div class="cr-terminal-output">'+output+'</div><form data-lab-terminal-form class="cr-terminal-form"><span>›</span><input data-lab-terminal autocomplete="off" spellcheck="false" placeholder="help / filter tcp / inspect 01"><button>RUN</button></form></div>');
}
...
function crEvidence(l,w){
 const data=Array.isArray(l?.evidence)?l:getLabDetails(l?.id),q=w.query.toLowerCase();
 const rows=data.evidence.map((x,i)=>({x,i})).filter(o=>!q||o.x.toLowerCase().includes(q));
 return '<div class="cr-toolbar"><div class="cr-search"><span>⌕</span><input data-lab-query placeholder="Filter evidence…" value="'+esc(w.query)+'"></div><span class="cr-count">'+rows.length+'/'+data.evidence.length+' EVENTS</span></div><div class="cr-table">'+rows.map(o=>'<button class="cr-row '+(o.i===w.selectedEvidence?"selected":"")+'" data-lab-select="'+o.i+'"><span>'+String(o.i+1).padStart(2,"0")+'</span><code>'+esc(o.x)+'</code><i>INSPECT</i></button>').join("")+'</div>';
}
function crInspector(l,w){
 const data=Array.isArray(l?.evidence)?l:getLabDetails(l?.id),x=data.evidence[w.selectedEvidence]||data.evidence[0]||"No evidence selected",locked=(w.locker||[]).includes(w.selectedEvidence);
 return crPanel("EVIDENCE INSPECTOR","SELECTED EVENT",'<div class="cr-inspector"><div class="cr-signal">EVENT '+String(w.selectedEvidence+1).padStart(2,"0")+'</div><code>'+esc(x)+'</code><p>Select observations and explain why they matter. NorthStar does not pre-label the answer for you.</p><div class="cr-facts"><span><b>TYPE</b> SIMULATED</span><span><b>SOURCE</b> LAB DATASET</span><span><b>TRUST</b> CONTROLLED</span></div><button class="cr-lock '+(locked?"locked":"")+'" data-lab-lock="'+w.selectedEvidence+'">'+(locked?"✓ IN EVIDENCE LOCKER":"＋ ADD TO EVIDENCE LOCKER")+'</button></div>');
}
function crEvidenceLocker(l,w){
 const data=getLabDetails(l.id),items=(w.locker||[]).map(n=>({n,x:data.evidence[n]})).filter(o=>o.x);
 return crPanel("EVIDENCE LOCKER",String(items.length).padStart(2,"0")+" ITEMS",'<div class="cr-locker">'+(items.length?items.map(o=>'<button class="cr-locker-item" data-lab-select="'+o.n+'"><span>EVT-'+String(o.n+1).padStart(2,"0")+'</span><code>'+esc(o.x)+'</code><i>OPEN</i></button>').join(""):'<div class="cr-empty-locker">Select an observation and add it here. Evidence stays local to this lab.</div>')+'</div>');
}
function crSpecializedInstrument(l,w){
 const id=l.id,d=getLabDetails(id),e=d.evidence||[],sel=e[w.selectedEvidence]||e[0]||"";
 if(id==="packet-recon"||id==="foundation-tcp"||id==="foundation-ip"){
  return '<div class="cr-workstation-grid">'+crPanel(id==="packet-recon"?"PACKET CAPTURE":"NETWORK OBSERVATORY","PCAP // SIMULATED",'<div class="cr-capture-head"><div><b>'+e.length*178+'</b><span>PACKETS LOADED</span></div><div><b>TCP / UDP</b><span>PROTOCOLS</span></div><div><b>'+String(w.selectedEvidence+1).padStart(2,"0")+'</b><span>SELECTED</span></div></div>'+crEvidence(l,w),"wide")+crPanel("PACKET INSPECTOR","LAYER VIEW",'<div class="cr-packet-detail"><div class="packet-row"><span>SUMMARY</span><b>'+esc(sel)+'</b></div><div class="packet-row"><span>NETWORK</span><b>SIMULATED IPv4 · CONTROLLED FLOW</b></div><div class="packet-row"><span>TRANSPORT</span><b>TCP · FLAGS / PORT CONTEXT</b></div><div class="packet-row"><span>PAYLOAD</span><b>REDACTED TRAINING DATA</b></div></div>')+crPanel("CONNECTION MAP","FLOW CORRELATION",'<div class="cr-connection-map"><span>CLIENT</span><i>──── TCP ────</i><span>DESTINATION</span></div>')+'</div>';
 }
 if(id==="tls-inspector"){
  return '<div class="cr-workstation-grid">'+crPanel("TLS HANDSHAKE","SEQUENCE // SIMULATED",'<div class="cr-handshake">'+["ClientHello · SNI","ServerHello · TLS 1.3","Certificate · CN / issuer","Validation · chain","Finished · encrypted data"].map((x,i)=>'<div class="'+(i<=w.selectedEvidence?"active":"")+'"><span>0'+(i+1)+'</span><b>'+x+'</b><small>'+((e[i]||"").split(" · ").slice(1).join(" · ")||"Observation available")+'</small></div>').join("")+'</div>',"wide")+crPanel("CERTIFICATE INSPECTOR","TRUST EVIDENCE",'<div class="cr-cert"><div><span>SUBJECT</span><b>training.northstar.local</b></div><div><span>ISSUER</span><b>NorthStar Lab CA</b></div><div><span>PROTOCOL</span><b>TLS 1.3</b></div><div><span>VALIDATION</span><b>TRUSTED IN LAB STORE</b></div><p>Trust is contextual. Verify hostname/SAN and validation policy before generalizing.</p></div>')+'</div>';
 }
 if(id==="http-request-lab"||id==="web-surface"||id==="auth-boundary"){
  return '<div class="cr-workstation-grid">'+crPanel("HTTP TRANSACTION","REQUEST / RESPONSE",'<div class="cr-http-split"><div><span>REQUEST</span><code>'+esc(sel)+'</code><pre>GET /api/profile HTTP/1.1\nCookie: session=LAB-7F3\nAccept: application/json</pre></div><div><span>RESPONSE</span><code>HTTP/1.1 403 Forbidden</code><pre>Set-Cookie: session=LAB-7F3\nContent-Type: application/json\nCache-Control: no-store</pre></div></div>',"wide")+crPanel("ROUTE INVENTORY","TRUST BOUNDARY",'<div class="cr-route-grid">'+["PUBLIC","AUTHENTICATED","PRIVILEGED"].map((x,i)=>'<div><span>'+x+'</span><b>'+[2,2,1][i]+' routes</b></div>').join("")+'</div>')+'</div>';
 }
 if(id==="windows-event-hunt"||id==="endpoint-process-hunt"||id==="detection-drill"||id==="log-triage"||id==="dns-hunt"||id==="linux-process"){
  return '<div class="cr-workstation-grid">'+crPanel("TELEMETRY STREAM","EVENTS // SIMULATED",'<div class="cr-telemetry">'+e.map((x,i)=>'<button class="'+(i===w.selectedEvidence?"selected":"")+'" data-lab-select="'+i+'"><span>'+String(i+1).padStart(2,"0")+'</span><code>'+esc(x)+'</code><i>EVENT</i></button>').join("")+'</div>',"wide")+crPanel("PROCESS / ENTITY TREE","CORRELATION",'<div class="cr-tree"><div>USER <b>→</b> '+(id.includes("windows")?"WINWORD / EXPLORER":"EXPLORER")+'</div><div class="indent">PROCESS <b>→</b> POWERSHELL / AGENT</div><div class="indent2">NETWORK <b>→</b> SIMULATED DESTINATION</div></div>')+crPanel("DETECTION CONTEXT","ANALYST VIEW",'<div class="cr-detection-context"><span>BASELINE</span><span>SEQUENCE</span><span>GAP</span><p>Correlate identity, process and network evidence before assigning a disposition.</p></div>')+'</div>';
 }
 if(id==="cloud-iam-review"){
  return '<div class="cr-workstation-grid">'+crPanel("IDENTITY GRAPH","IAM // SIMULATED",'<div class="cr-iam-graph"><span>analyst01<small>ReadOnly</small></span><i>→</i><span>deploy01<small>Deploy</small></span><i>→</i><span>Admin<small>AssumeRole</small></span></div>',"wide")+crPanel("AUDIT TRAIL","ACCESS EVENTS",'<div class="cr-audit">'+e.map((x,i)=>'<button class="'+(i===w.selectedEvidence?"selected":"")+'" data-lab-select="'+i+'"><b>0'+(i+1)+'</b><code>'+esc(x)+'</code></button>').join("")+'</div>')+crPanel("LEAST PRIVILEGE","REVIEW",'<div class="cr-control"><b>QUESTION</b><p>Is each permission required by the documented workflow?</p><span>Compare role → source identity → change record.</span></div>')+'</div>';
 }
 if(id==="container-security-room"){
  return '<div class="cr-workstation-grid">'+crPanel("IMAGE MANIFEST","SUPPLY CHAIN // SIMULATED",'<div class="cr-manifest"><div><span>IMAGE</span><b>api:v4</b></div><div><span>DIGEST</span><b>sha256:LAB123</b></div><div><span>BASE</span><b>debian</b></div><div><span>USER</span><b>root</b></div></div>',"wide")+crPanel("RUNTIME ROOM","CONTAINER TELEMETRY",'<div class="cr-runtime">'+e.map((x,i)=>'<button class="'+(i===w.selectedEvidence?"selected":"")+'" data-lab-select="'+i+'"><span>0'+(i+1)+'</span><code>'+esc(x)+'</code></button>').join("")+'</div>')+crPanel("EXPOSURE MAP","SERVICE BOUNDARY",'<div class="cr-exposure"><span>HOST</span><i>↔</i><b>8080</b><i>↔</i><span>API CONTAINER</span></div>')+'</div>';
 }
 if(id==="phishing-triage"){
  return '<div class="cr-workstation-grid">'+crPanel("MAIL HEADER ANALYZER","MESSAGE // SIMULATED",'<div class="cr-mail"><div><span>VISIBLE FROM</span><b>payroll-update@payr0ll.example</b></div><div><span>RETURN-PATH</span><b>bounce@mailer.example</b></div><div><span>SPF</span><b class="good">PASS</b></div><div><span>DKIM</span><b class="good">PASS</b></div><div><span>DMARC</span><b class="warn">FAIL ALIGNMENT</b></div></div>',"wide")+crPanel("URL INSPECTOR","PASSIVE ANALYSIS",'<div class="cr-url"><span>DESTINATION</span><code>https://payr0ll.example/verify</code><p>No payload execution. Compare visible domain, authenticated domains and user context.</p></div>')+crPanel("TRIAGE SIGNAL","ANALYST VIEW",'<div class="cr-signal-grid"><span>IDENTITY</span><span>AUTHENTICATION</span><span>URL</span><span>CONTEXT</span></div>')+'</div>';
 }
 if(id==="ai-prompt-defense"){
  return '<div class="cr-workstation-grid">'+crPanel("PROMPT STACK","TRUST BOUNDARY // SIMULATED",'<div class="cr-prompt-stack">'+["SYSTEM POLICY","USER MESSAGE","UNTRUSTED TOOL RESULT","MODEL OUTPUT"].map((x,i)=>'<div class="layer layer-'+i+'"><span>0'+(i+1)+'</span><b>'+x+'</b><small>'+esc(e[i]||"Controlled observation")+'</small></div>').join("")+'</div>',"wide")+crPanel("TRUST BOUNDARY","AUTHORITY MODEL",'<div class="cr-trust-boundary"><span>SYSTEM</span><i>HIGHER AUTHORITY</i><span>USER / TOOL DATA</span><i>UNTRUSTED CONTENT</i><span>MODEL</span></div>')+crPanel("EVALUATION CONSOLE","DEFENSIVE TEST",'<div class="cr-eval"><div><b>CANARY</b><span>HIDDEN-LAB-SECRET</span></div><div><b>EXPECTED</b><span>REFUSE SECRET DISCLOSURE</span></div><div><b>OBSERVED</b><span>COMPARE MODEL OUTPUT</span></div></div>')+'</div>';
 }
 return '<div class="cr-workstation-grid">'+crPanel("LAB TELEMETRY","CONTROLLED DATASET",crEvidence(l,w),"wide")+crInspector(l,w)+'</div>';
}
function crInstrument(l,w){return crSpecializedInstrument(l,w);}
function cyberHeader(l){return '<div class="cr-header"><div><button class="back" data-route="labs">← CyberRange</button><div class="cr-kicker">NORTHSTAR // CYBERRANGE 3.1</div><h1>'+esc(l.title)+'</h1><p>'+esc(l.subtitle)+' · <b>'+esc(l.level)+'</b> · '+esc(l.duration||"Practice")+'</p></div><div class="cr-live"><i></i><b>SIMULATION</b><span>LOCAL ONLY</span><small>LAB '+String(state.selectedLab+1).padStart(3,"0")+'</small></div></div>';}
function crOperatorHud(l,d,w,answered,pct){
 const locked=(w.locker||[]).length;
 return '<div class="cr-operator-hud"><div class="hud-cell"><span>RANGE</span><b>ISOLATED</b><small>NO EXTERNAL IO</small></div><div class="hud-cell"><span>OBJECTIVE</span><b>'+esc(String(l.objective||"Investigation"))+'</b><small>OBSERVE · CORRELATE · REPORT</small></div><div class="hud-cell"><span>EVIDENCE</span><b>'+locked+' LOCKED / '+d.evidence.length+'</b><small>LOCAL EVIDENCE STORE</small></div><div class="hud-cell hud-progress"><span>PROGRESS</span><b>'+pct+'%</b><div><i style="width:'+pct+'%"></i></div></div></div>';
}
const views={
 home:()=>{
